...
class Cms_menuitems_model extends CI_Model {
    ...
    function getSubMenu($slug){
        try{
        $sql = "SELECT F2.menu, F2.slug, F2.open_new_tab, F2.ext_url, F2.ext_url_link, front_cms_pages.url as `page_url` 
                    FROM front_cms_menu_items AS F1
                    INNER JOIN front_cms_menu_items AS F2
                    ON (F1.parent_id = F2.parent_id)
    ...
    function dyn_menu($parent_array, $sub_array, $qs_val = 'menu', $main_id = 'nav', $sub_id = 'subnav', $extra_style = 'foldout') {
        ...
        foreach ($parent_array as $pkey => $pval) {
            ...
            foreach ($sub_array as $skey=>$sval) {
                ...
                if ($pkey == $sval['parent']) {
                    $array[$pval['id']]['submenus'][$skey] = array(
                        'id' => $sval['id'],
                        'slug' => $sval['link'],
                        'menu' => $sval['label'],
                    ...
                    foreach($sub_array as $ckey=>$cval){
                        if($cval['parent']==$sval['id']){
                           $array[$pval['id']]['submenus'][$skey]['submenus'][] = array(
                        'id' => $cval['id'],
                        'slug' => $cval['link'],
                        'menu' => $cval['label'],
                        'page_id' => $cval['page_id'],
                        'is_homepage' => $cval['is_homepage'],
                        'ext_url' => $cval['ext_url'],
                        'ext_url_link' => $cval['ext_url_link'],
                        'open_new_tab' => $cval['open_new_tab'],
                        'publish' => $cval['publish'],
                        'page_slug' => $cval['page_slug'],
                        'page_url' => $cval['page_url'],
                    );
                        }
                    }
