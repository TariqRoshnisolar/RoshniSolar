...
class Cms_program_model extends CI_Model {
    ...
    public function front_cms_program_photos($program_id) {
        $this->db->select('front_cms_media_gallery.*, front_cms_program_photos.is_slider');
    ...
    public function update_batch($data, $contents) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(FALSE); # See Note 01. If you wish can remove as well 
        $this->db->where('id', $data['id']);
        $this->db->update('front_cms_programs', $data);
        ...
        if (isset($contents) && !empty($contents)) {           
            ...
            $this->db->where('program_id', $data['id'])->delete('front_cms_program_photos');
            $this->db->insert_batch('front_cms_program_photos', $contents);
        }
    ...
    public function getSilderImages(){
        $this->db->select('front_cms_media_gallery.*');
        $this->db->from('front_cms_media_gallery');
        $this->db->join('front_cms_program_photos', 'front_cms_program_photos.media_gallery_id=front_cms_media_gallery.id');
        $this->db->where('front_cms_program_photos.is_slider', 1);
        $query = $this->db->get();
        if($query->num_rows() >0){
            return $query->result_array();
        }
    ...
    public function removeBySlug($slug, $type) {
        ...
        $this->db->update('front_cms_programs', ['is_active'=>'no']);
        //$this->db->delete('front_cms_programs');
    }
