...
class Staff extends Admin_Controller {
    ...
    function create() {
        ...
        $this->form_validation->set_rules('name', 'Name', 'alpha_dash|trim|required|xss_clean');
        $this->form_validation->set_rules('role', 'Role', 'trim|required|xss_clean');
        $this->form_validation->set_rules('gender', 'Gender', 'trim|required|xss_clean');
        $this->form_validation->set_rules('dob', 'Date of Birth', 'trim|required|xss_clean');

        $this->form_validation->set_message('alpha_dash', 'Space not allowed in name');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $username_suffix = $this->role->get_random_password($chars_min = 3, $chars_max = 4, $use_upper_case = false, $include_numbers = true, $include_special_chars = false);
            
            $username = strtolower($name.$username_suffix);
            ...
            if ($staff_id) {
                ...
                $teacher_login_detail = array('id' => $staff_id, 'credential_for' => 'staff', 'username' => $username, 'password' => $password, 'contact_no' => $contact_no, 'email' => $email);
    ...
    function edit($id) {
        ...
        $data['staff'] = $staff;
        $data["contract_type"] = $this->contract_type;
        //$data["winglist"] = $this->wing_model->get();
  ...
  $this->form_validation->set_rules('new_pass', 'New password', 'trim|required|xss_clean|matches[confirm_pass]');
        ...
        if(!empty($id)){
            $newdata = array(
                ...
                'cleanpass' => $this->input->post('new_pass')
            );
    ...
    function stafflogincredential(){
        ...
        $this->session->set_userdata('active_menu', 'hrmenu');
        //$this->session->set_userdata('top_menu', 'HR');
        //$this->session->set_userdata('sub_menu', 'admin/staff/stafflogincredential');

        $resultlist = $this->staff_model->getAll();

        $data['resultlist'] = $resultlist;
        
        $this->load->view('layout/header');
        $this->load->view('admin/staff/stafflogincredential', $data);
        $this->load->view('layout/footer');
    }
