...
class User extends Student_Controller {
    ...
    function dashboard() {
        ...
        $student_fee_detail = $this->studentfee_model->studentfeedetail($student_id, $student_session_id, $currentsession);
        
        $classsectionid = $this->student_model->getclasssectionbystuid($student_id);

        $feetype = 1;

        $feegroupid = $this->studentfee_model->getstudentfeegroup($classsectionid['id']);    

        $student_pending_fee_detail = $this->studentfee_model->getpendinginstallment($student_id, $currentsession, $feetype, $feegroupid);

        $data['student_pending_fee_detail'] = $student_pending_fee_detail;

        $data['student_fee_detail'] = $student_fee_detail;
    ...
    function getfees() {
        ...
        $classsectionid = $this->student_model->getclasssectionbystuid($id);
        $data["classsectionid"] = $classsectionid['id'];
