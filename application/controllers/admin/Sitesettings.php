...
class Sitesettings extends CI_Controller {
	...
	public function manageSettings(){
		$data = array('viewPage'=>'site-settings/manage','jsFiles'=>array('moment.min','daterangepicker.min','site-settings'),'pageTitle'=>'Site Settings','activeMenus'=>array('settings','site-settings'),'cssFiles'=>array('daterangepicker'));
	...
	public function saveData(){
		...
		$opening_date = str_replace('/','-',$data['opening_date']);
		$closing_date = str_replace('/','-',$data['closing_date']);
		$last_date = str_replace('/','-',$data['last_date']);
		$data['opening_date'] = date('Y-m-d',strtotime($opening_date));
		$data['closing_date'] = date('Y-m-d',strtotime($closing_date));
		$data['last_date'] = date('Y-m-d',strtotime($last_date));
		$data['modified_by'] = $this->session->userdata('user_id');
		$data['modified_at'] = date('Y-m-d H:i:s');
		$data['instant_close_timer'] = isset($data['instant_close_timer'])?1:2;
