...
function searchFilter(page_num,loadingType) {
  ...
  $.ajax({
    ...
    data:'page='+page_num+'&sortBy='+sortBy+'&sortByField='+sortByField+'&perPage='+perPage+'&keyword='+keyword+'&startEnd='+startEnd+'&posts='+posts+'&job_state='+job_state+'&centre='+centre+'&payment_status='+payment_status,
    beforeSend: function () {
...
function exportCSV(){
  $("#form-search1").attr('action',BASE_URL+'users/export-data');
  $("#form-search1").submit(); 
  $("#form-search1").attr('action',BASE_URL+'admin/users');    
  return false;    
}