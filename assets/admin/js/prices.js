function saveData(){
  ...
  $.ajax({
    type: "POST",
    url: BASE_URL +'admin/price-save-data',
    data: formData,
    ...
    success:function(result){
      ...
      if(data.status == 1){
        ...
        toastr.success(data.msg, 'Saved', {timeOut: 1000,progressBar:true,onHidden:function() { window.location.href = BASE_URL+'admin/prices'; }   });
      } else {
...
function searchFilter(page_num,loadingType) {
  ...
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/price-search-data/'+page_num,
    data:'page='+page_num+'&sortBy='+sortBy+'&sortByField='+sortByField+'&perPage='+perPage+'&keyword='+keyword+'&startEnd='+startEnd,
    beforeSend: function () {
...
$(document).ready(function(){
  ...
  var param = url.split('/');
  if( param[param.length - 1] && param[param.length - 1] == 'prices' ){
    searchFilter('',1);
  ...
  $(".cancelButton").click(function(){
    window.location.href=BASE_URL+"admin/prices";
  });
